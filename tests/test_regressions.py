...
class WorkflowPolicyTests(unittest.TestCase):
    ...
    def test_oled_agent_ci_has_manual_ui_audit_acceptance_job(self) -> None:
        ...
        workflow = self._workflow_path(repo_root)
        content = workflow.read_text(encoding="utf-8")
        self.assertIn("run_ui_audit_acceptance:", content)
        self.assertIn("ui-audit-acceptance:", content)
        self.assertIn("acceptance ui-audit (manual)", content)
        self.assertIn("github.event.inputs.run_ui_audit_acceptance == 'true'", content)
        self.assertIn("Run ui-audit acceptance", content)
    ...
    def test_oled_agent_ci_has_ui_acceptance_bundle_entry_and_summary_job(self) -> None:
        ...
        self.assertIn("bundle_failure_names: ${{ steps.bundle_outputs.outputs.bundle_failure_names }}", content)
        self.assertIn("bundle_first_failure: ${{ steps.bundle_outputs.outputs.bundle_first_failure }}", content)
        ...
        self.assertIn("summary_bundle_first_failure", content)
...
class PlanProgressAssetsTests(unittest.TestCase):
    ...
    def test_collect_real_chain_evidence_script_writes_release_evidence(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIsInstance(baseline_ctx, dict)
            self.assertEqual(baseline_ctx.get("base_task_id"), "demo_task")
            self.assertEqual(baseline_ctx.get("baseline_status"), "missing")
            self.assertEqual(baseline_ctx.get("archive_manifest_status"), "missing")
            self.assertEqual(baseline_ctx.get("archive_release_gate_status"), "missing")

    def test_collect_real_chain_evidence_script_includes_baseline_archive_context(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as td:
            td_path = Path(td)
            task_id = "demo_base_r2"
            run_dir = td_path / "runs" / "agent" / task_id
            run_dir.mkdir(parents=True, exist_ok=True)
            plan_path = run_dir / "plan.json"
            execution_path = run_dir / "execution.json"
            ...
            base_dir = td_path / "runs" / "agent" / "demo_base"
            archive_dir = td_path / "runs" / "archive" / "demo_base"
            base_dir.mkdir(parents=True, exist_ok=True)
            archive_dir.mkdir(parents=True, exist_ok=True)
            (base_dir / "baseline_summary.json").write_text(
                json.dumps({"status": "pass", "run_count": 3}, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
                ...
                json.dumps(
                    {
                        ...
                        "release_gate_summary": {
                            ...
                            "pass_count": 2,
                            "fail_count": 1,
                            "failures": ["run2: guardrails_strict_status!=pass", "run2: fallback_used"],
                        },
                    },
                    ensure_ascii=False,
                    indent=2,
            ...
            baseline_ctx = evidence.get("baseline_context")
            self.assertIsInstance(baseline_ctx, dict)
            self.assertEqual(baseline_ctx.get("base_task_id"), "demo_base")
            self.assertEqual(baseline_ctx.get("baseline_status"), "pass")
            self.assertEqual(baseline_ctx.get("baseline_run_count"), 3)
            self.assertEqual(baseline_ctx.get("archive_manifest_status"), "archived")
            self.assertEqual(baseline_ctx.get("archive_release_gate_status"), "fail")
            self.assertEqual(baseline_ctx.get("archive_release_gate_checked_runs"), 3)
            self.assertEqual(baseline_ctx.get("archive_release_gate_pass_count"), 2)
            self.assertEqual(baseline_ctx.get("archive_release_gate_fail_count"), 1)
            self.assertEqual(
                baseline_ctx.get("archive_release_gate_failures_preview"),
                ["run2: guardrails_strict_status!=pass", "run2: fallback_used"],
            )
            md = evidence_md.read_text(encoding="utf-8")
            self.assertIn("## Baseline Context", md)
            self.assertIn("archive_release_gate_status: `fail`", md)
...
class UiPrototypeTests(unittest.TestCase):
    ...
    def test_ui_html_contains_prompt_history_and_shortcut_hints(self) -> None:
        ...
        self.assertIn("chat_quick_chips", html)
        self.assertIn("quick_chip_target_btn", html)
        self.assertIn("quick_chip_candidate_patch_btn", html)
        self.assertIn("quick_chip_step_clean_btn", html)
        self.assertIn("quick_chip_step_train_btn", html)
        self.assertIn("quick_chip_web_hint_btn", html)
        self.assertIn("applyQuickChatChip(", html)
    ...
    def test_ui_html_contains_workspace_hud_and_web_search_action(self) -> None:
        ...
        self.assertIn("simple_project_strip", html)
        self.assertIn("simple_project_picker", html)
        self.assertIn("simple_new_project_id", html)
        self.assertIn("simple_project_status", html)
        self.assertIn("switchSimpleProjectFromPicker()", html)
        self.assertIn("createProjectFromSimpleBar()", html)
        self.assertIn("setSimpleProjectDraftFromCurrent()", html)
        self.assertIn("suggestSimpleProjectDraft()", html)
        self.assertIn("syncSimpleProjectBar(", html)
        self.assertIn("suggestSimpleProjectId(", html)
        self.assertIn("hud_project_id", html)
        self.assertIn("current_task_id_hud", html)
        self.assertIn("runtime_elapsed_hud", html)
        ...
        self.assertIn("web_preset_patents_btn", html)
        ...
        self.assertIn("pending_hint_run_status", html)
        self.assertIn("pending_hint_next_actions", html)
        self.assertIn("pending_auto_actions", html)
        self.assertIn("pending_auto_fill_btn", html)
        self.assertIn("pending_auto_patch_btn", html)
        self.assertIn("pending_auto_resume_btn", html)
        self.assertIn("renderPendingAutoActions(", html)
        self.assertIn("pendingAutoFillCandidateData(", html)
        self.assertIn("pendingAutoFillAndPatch(", html)
        self.assertIn("pendingAutoFillAndResume(", html)
        ...
        self.assertIn("control_center_drawer", html)
        self.assertIn("control_center_status", html)
        self.assertIn("control_center_mode_btn", html)
        self.assertIn("control_center_mode_state", html)
        self.assertIn("toggleControlCenterMode()", html)
        self.assertIn("applyControlCenterMode(", html)
        self.assertIn("control_center_audit_empty", html)
        self.assertIn("control_center_audit_list", html)
        self.assertIn("control_center_audit_filter", html)
        self.assertIn("control_center_audit_sort", html)
        self.assertIn("control_center_audit_query", html)
        self.assertIn("control_center_audit_copy_btn", html)
        self.assertIn("control_center_audit_meta", html)
        self.assertIn("control_center_audit_export_btn", html)
        self.assertIn("control_center_audit_clear_btn", html)
        self.assertIn("onControlCenterAuditFilterChanged()", html)
        self.assertIn("onControlCenterAuditSortChanged()", html)
        self.assertIn("onControlCenterAuditQueryChanged()", html)
        self.assertIn("exportControlCenterAudit()", html)
        self.assertIn("copyControlCenterAudit()", html)
        self.assertIn("clearControlCenterAudit()", html)
        self.assertIn("appendControlCenterAudit(", html)
        self.assertIn("renderControlCenterAudit(", html)
        ...
        self.assertIn("guided_recovery", html)
        self.assertIn("control-center-grid", html)
        self.assertIn("quality_guard_card", html)
        self.assertIn("quality_guard_text", html)
        self.assertIn("quality_guard_issues", html)
        self.assertIn("quality_guard_suggest_btn", html)
        self.assertIn("computeQualitySuggestion(", html)
        ...
        self.assertIn("agent_guidance_card", html)
        self.assertIn("agent_guidance_text", html)
        self.assertIn("agent_guidance_detail", html)
        self.assertIn("agent_guidance_action_btn", html)
        self.assertIn("renderAgentGuidanceCard(", html)
        self.assertIn("applyAgentGuidanceAction()", html)
        self.assertIn("agent_recovery_card", html)
        self.assertIn("agent_recovery_text", html)
        self.assertIn("agent_recovery_preview_btn", html)
        self.assertIn("agent_recovery_apply_btn", html)
        self.assertIn("renderAgentRecoveryCard(", html)
        self.assertIn("runGuidedRecovery(", html)
        ...
        self.assertIn("downloadTaskArtifactPack()", html)
        ...
        self.assertIn("simpleAttachPath()", html)
        ...
        self.assertIn("toggleFocusMode()", html)
        self.assertIn("output_view_state", html)
        self.assertIn("output_view_mode", html)
        self.assertIn("ui_mode_simple_btn", html)
        self.assertIn("ui_mode_advanced_btn", html)
        self.assertIn("setPrimaryUiMode('simple')", html)
        self.assertIn("setPrimaryUiMode('advanced')", html)
        self.assertIn("syncPrimaryUiModeSwitch(", html)
        self.assertIn("onOutputViewModeChanged()", html)
        self.assertIn("toggleOutputViewMode()", html)
        self.assertIn("right_retry_controls", html)
        ...
        self.assertIn("session_filter_readiness", html)
        self.assertIn("session_filter_resume_mode", html)
        self.assertIn("quickFilterByReleaseGate(", html)
        self.assertIn("quickFilterByReadiness(", html)
        self.assertIn("quickFilterByResume(", html)
        ...
        self.assertIn("loadCurrentMemoryContext()", html)
        self.assertIn("appendTopMemoryFactsToNotes()", html)
        self.assertIn("sendFirstMemoryFactToChat()", html)
    ...
    def test_ui_html_contains_workspace_url_controls(self) -> None:
        ...
        self.assertIn("openWorkspaceWindowForProject(", html)
        self.assertIn("copyWorkspaceLink()", html)
        self.assertIn("readProjectIdFromUrl()", html)
        self.assertIn("syncWorkspaceUrl(", html)
        self.assertIn("openWorkspaceWindowForProject(", html)
        self.assertIn("openPinnedSessionWindows()", html)
        self.assertIn("openFilteredSessionWindows()", html)
        self.assertIn("loadUiPrefs()", html)
        self.assertIn("applyFocusMode(", html)
        ...
        self.assertIn("body.output-simple-mode .panel.left-drawer", html)
        ...
        self.assertIn("quickFilterByReadiness('fail')", html)
        self.assertIn("quickFilterByReadiness('warn')", html)
        self.assertIn("quickFilterByReadiness('pass')", html)
        self.assertIn("quickSortPriority()", html)
        self.assertIn("readiness_fail_first", html)
        self.assertIn("projectReadinessStatus(", html)
        ...
        self.assertIn("toggleBatchCompareDetails()", html)
        self.assertIn("downloadLatestBatchCompare('json')", html)
        self.assertIn("downloadLatestBatchCompare('txt')", html)
        self.assertIn("copyBatchCompareDetails()", html)
        self.assertIn("copyBatchComparePath(", html)
        ...
        self.assertIn("readBatchComparePathFilter()", html)
        ...
        self.assertIn("renderBatchCompareChangedPaths(", html)
        self.assertIn("renderBatchCompareSelectedDiff(", html)
        self.assertIn("getBatchCompareChangedRows(", html)
        self.assertIn("setBatchCompareDetailsVisible(", html)
        ...
        self.assertIn("project_batch_compare_summary", html)
        self.assertIn("project_batch_compare_details", html)
        self.assertIn("project_batch_compare_paths", html)
        self.assertIn("project_batch_compare_selected_diff", html)
        self.assertIn("batch_compare_toggle_btn", html)
        self.assertIn("batch_compare_path_filter", html)
        ...
        self.assertIn("renderResumeDiagnostics(", html)
        self.assertIn("resume_visibility", html)
        self.assertIn("resume(skip/partial/full/no)", html)
        self.assertIn("resume_diag_box", html)
        self.assertIn("resume_diag_steps", html)
    ...
    def test_ui_task_artifact_links_endpoint_returns_paths(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(str(payload.get("artifact_pack_api") or ""), f"/api/task/{task_id}/artifact-pack")
    ...
    def test_ui_task_artifact_pack_downloads_zip(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (run_dir / "artifacts").mkdir(parents=True, exist_ok=True)
            (run_dir / "artifacts" / "web_evidence.json").write_text(
                json.dumps({"status": "pass", "query": "demo"}, ensure_ascii=False) + "\n",
                encoding="utf-8",
            )
            with mock.patch.object(ui_app_mod, "REPO_ROOT", root):
                client = ui_app_mod.app.test_client()
                resp = client.get(f"/api/task/{task_id}/artifact-pack?artifacts=decision_summary,execution,web_evidence")
            self.assertEqual(resp.status_code, 200)
            self.assertIn("application/zip", str(resp.content_type or ""))
            disp = str(resp.headers.get("Content-Disposition") or "")
            self.assertIn("attachment;", disp)
            self.assertIn(f"agent4mat-task-{task_id}-artifact-pack-", disp)
            buf = io.BytesIO(resp.get_data())
            with zipfile.ZipFile(buf, mode="r") as zf:
                ...
                self.assertTrue(any(name.endswith("artifacts/web_evidence/web_evidence.json") for name in names))
    ...
    def test_ui_task_artifact_pack_rejects_invalid_artifact_name(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            task_id = "ui_artifact_pack_invalid_case"
            run_dir = root / "runs" / "agent" / task_id
            run_dir.mkdir(parents=True, exist_ok=True)
            with mock.patch.object(ui_app_mod, "REPO_ROOT", root):
                client = ui_app_mod.app.test_client()
                resp = client.get(f"/api/task/{task_id}/artifact-pack?artifacts=decision_summary,bad_name")
            self.assertEqual(resp.status_code, 400)
            payload = resp.get_json()
            self.assertEqual(payload.get("status"), "fail")
